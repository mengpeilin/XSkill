...
def detect_moving_objects_array(arr, obs_indices, threshold=0.005):
    ...
    max_required_index = max(int(np.max(indices)) for indices in obs_indices.values())
    if arr.shape[1] <= max_required_index:
        return None
...
def plot_proto_task_relation(demo_type="human", cfg=None):
    ...
    for f in all_folders:
        moved_obj_path = os.path.join(encode_path, f, "moved_obj.json")
        if not os.path.exists(moved_obj_path):
            continue
        ...
        with open(moved_obj_path, "r") as file:
    ...
    if not labels:
        raise ValueError(
            "No moved_obj.json files were found. This plot requires kitchen-style object state annotations."
        )
